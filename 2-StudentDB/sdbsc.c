 ...
 *
 * Funtionality Calculate offset based on ID. Seek to the corect position, read the student record, check for read errors, check if it was found empty, and return no error if checks fail
*/
 ...
 *
 * Functionality: Check if student already exists. Initialize new student. Seek to positon and write empty record 
*/
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    int rc = get_student(fd, id, &existing);
    if (rc == NO_ERROR) {
    ...
    if (write(fd, &new, STUDENT_RECORD_SIZE) != STUDENT_RECORD_SIZE) {
 ...
 *            M_ERR_DB_WRITE     error writing to db file (adding student)
 *
 * Functionality: Search for student, seek back to student postion, write empty record, 
*/
 ...
 *            M_ERR_DB_WRITE   error writing to db file (adding student)
 *
 * Funtionality: Go to the start of the file and read records until the end of file
*/
 ...
 *            M_ERR_DB_READ    error reading or seeking the database file
 *
 * Functionality: Go to the start of the file and read and print the records
*/
int print_db(int fd)
{
    student_t student = {0};
 ...
 *
 * Fucntionality: Create a temp db file, go to the start of the original file, read and copy valid records,
close old and replace as new,reopen  
*/
